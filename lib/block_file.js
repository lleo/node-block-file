...
function BlockFile_create(filename, metaProps_) {
  ...
  return vowfs.open(filename, 'wx+')
         .then(
           function(fd_){
             fd = fd_
             return vowfs.write( fd
                               , mdBuf         /*buffer*/
                               , 0             /*buffer offset*/
  ...
  return vowfs.stat(filename)
         ...
         .then(
           function(){
             ...
             return vowfs.open(filename, 'r+')
           })
         .then(
           function(fd_){
             fd = fd_
             return vowfs.read( fd
                              , mdBuf           /*buffer*/
                              , 0               /*buffer offset*/
...
function BlockFile__close() {
  ...
  return self.writeSegments()
         ...
         .then(function(){ return vowfs.close(self.fd) })
} //BlockFile__close()
...
function BlockFile__readSegment(segNum) {
  ...
  //console.error("BlockFile#_readSegment: segNum = %d", segNum)
...
function BlockFile__load(hdl) {
  ...
  //readp.then( doneNS )
  readp.then(function(){ doneNS() })

  loadp = readp.spread(function(bytesRead, buffer){
            loadp.fulfill([buffer, hdl])
          })
...
BlockFile.prototype.store = function store(buffer, hdl){
  ...
  //console.error("buffer.length/blockSize = %d/%d = %s"
  //             , buffer.length, this.props.blockSize
  //             , buffer.length / this.props.blockSize)

  hdl = hdl || this._reserve(numBlks)

  //console.error("***store hdl=%s", hdl)

  blkOff = this._calcBlkOff(hdl.segNum, hdl.blkNum)
  writep = vowfs.write( this.fd
                      , buffer
                      , 0             /*buffer position*/
...
//function BlockFile__store(buffer) {
//  var numBlks, len, segNum, hdl, blkOff, storep
//
//  numBlks = Math.ceil( buffer.length / this.props.blockSize )
//
//  hdl = this._reserve(numBlks)
//
//  storep = this._store(buffer, hdl)
//
//  return storep
//} //BlockFile__store()
//
...
//function BlockFile___store(buffer, hdl) {
//  var blkOff
//    , doneNS = bfStats.get("tt_store ns").start()
//    , writep, storep
//
...
//  writep.then(function(){ doneNS() })
//
//  return writep.spread(function(bytesWritten, buf){
//           return hdl
//         })
//} //BlockFile___store()
