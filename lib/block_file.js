...
function BlockFile__writeSegments(cb) {
  ...
  //log( "segs.length = %d; segNums=%j"
  //   , segs.length
  //   , u.map(segs, function(seg){ return seg.segNum }) )
...
function BlockFile__write(buffer, cb) {
  //A!assert.ok( Buffer.isBuffer(buffer) )
  //A!assert.equal(typeof cb, 'function')

  var self = this
    , numBlks, len, segNum, hdl, blkOff
