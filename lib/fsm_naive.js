...
function NaiveFSM(freeBlockMap) {
  //A!assert.equal(freeBlockMap.length, MAX_BLOCKNUM+1)
...
function NaiveFSM__alloc(reqSpanNum) {
  ...
  //A!Handle.assertValidSpanNum( reqSpanNum )
  ...
  if (spanNum > MAX_SPANNUM) {
    //throw new Error(format( "No spans available at %d thru %d"
    //                      , reqSpanNum, MAX_SPANNUM ))
...
function NaiveFSM__free(begBlkNum, spanNum) {
  //A!Handle.assertValidBlockNum(begBlkNum)
  ...
  if ( Handle.isValidBlockNum(hiBlkNum) /*in the correct range*/) {
    ...
    if ( hiSpanNum !== undefined ) {
      //A!Handle.assertValidSpanNum(hiSpanNum)
      ...
      //A!assert( idx !== len, format("hiBlkNum(%d) not found in this.spans[%d]"
      //A!                           , hiBlkNum, hiSpanNum))
  ...
  if ( Handle.isValidBlockNum(loBlkNum) /*in the correct range*/) {
    ...
    if ( loSpanNum !== undefined ) {
      //A!Handle.assertValidSpanNum(loSpanNum)
      ...
      //if (idx === len) /*not found*/
      //A!assert( idx !== len, format("loBlkNum(%d) not found in this.spans[%d]"
      //A!                           , loBlkNum, loSpanNum))
...
function NaiveFSM__insert(begBlkNum, spanNum) {
  ...
  //A!Handle.assertValidBlockNum(begBlkNum)
  //A!Handle.assertValidSpanNum(spanNum)

  len = this.spans[spanNum].length
  for (idx = 0; idx < len; idx += 1) {
    //A!assert.ok(this.spans[spanNum][idx] !== begBlkNum)
...
//function spanIntersects(begBlkNumA, spanNumA, begBlkNumB, spanNumB) {
//  var endBlkNumA = begBlkNumA + spanNumA
//    , endBlkNumB = begBlkNumB + spanNumB
//
//  if ((begBlkNumA <= begBlkNumB) && (begBlkNumB <= endBlkNumA)) return true
//  if ((begBlkNumA <= endBlkNumB) && (endBlkNumB <= endBlkNumA)) return true
//
//  if ((begBlkNumB <= begBlkNumA) && (begBlkNumA <= endBlkNumB)) return true
//  if ((begBlkNumB <= endBlkNumA) && (endBlkNumA <= endBlkNumB)) return true
//  //this last test won't match;
//  // because if begB<=endA<=endB
//  // then begA<=endB<=endA would have already matched
//
//  return false
//}
//
//NaiveFSM.prototype.isValid = NaiveFSM__isValid
//function NaiveFSM__isValid(begBlkNum, spanNum) {
//
//  //make sure span does not intersect with any other span
//  //maybe forAllSpans(function(span, spanNum, idx){...})
//  var tBegBlkNum, tSpanNum, idx
//  for (tSpanNum = MIN_SPANNUM; tSpanNum <= MAX_SPANNUM; tSpanNum += 1) {
//    for (idx=0; idx < this.spans[tSpanNum].length; idx += 1) {
//      tBegBlkNum = this.spans[tSpanNum][idx]
//
//      if ( spanIntersects(begBlkNum, spanNum, tBegBlkNum, tSpanNum) )
//        return false
//
