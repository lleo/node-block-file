...
Segment.prototype.sign = function() {
  if (this.props.checkSumBits === 16)
    signCRC16(this.buffer, 0)
  else if (this.props.checkSumBits === 32)
    signCRC32(this.buffer, 0)
  else
    throw new Error("only 16 & 32 checkSumBits are supported")
}

Segment.prototype.validate = function(){
  if (this.props.checkSumBits === 16)
    validateCRC16(this.buffer, 0)
  else if (this.props.checkSumBits === 32)
    validateCRC32(this.buffer, 0)
  else
    throw new Error("only 16 & 32 checkSumBits are supported")
}
