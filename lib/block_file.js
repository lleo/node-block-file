...
bfStats.createStat("tt_write_segment", Stats.TimerNS)
bfStats.createStat("tt_flush", Stats.TimerNS)
bfStats.createHOG("hog tt_write_segment", "tt_write_segment", Stats.semiLogNS)
bfStats.createHOG("hog tt_flush", "tt_flush", Stats.semiLogNS)
 ...
 */
BlockFile.prototype._initialize = function(stat, initcb){
 ...
 * @return {Boolean}
 */
BlockFile.prototype.setAppData = function(appDataBuf){
  var metaPropsLen = this.mdBuf.readUInt16BE(MD_MAP['metaPropsLen'])
    , avail = MD_BLOCKSIZE - MD_MAP['END_FIXED'] - metaPropsLen
...
BlockFile.prototype.flush = function(cb){
  var self = this
    , doneNS = bfStats.get("tt_flush").start()

//  console.warn("BlockFile#flush: called!")

  async.series( [ function(scb){ self._writeSegments(scb) }
                , function(scb){ self._writeHeader(scb)   }
 ...
 * @param {Function} cb cb(err)
 */
BlockFile.prototype._writeHeader = function(cb){
  var self = this
 ...
 */
BlockFile.prototype._addSegment = function(){
  var self = this
...
BlockFile.prototype._readSegment = function(segNum, cb){
  ...
  function finish(err, buffer) {
    ...
    //fresh means it's never been written to disk
    self.segments[segNum].fresh = false
...
BlockFile.prototype._writeSegment = function(segNum, cb){
  var self = this
    ...
    , doneNS = bfStats.get("tt_write_segment").start()

  //seg.sign()
  signCRC(seg.curBitfieldBuf, this.props)

  var bytesChanged = [] // [ [start, length], ... ]
    , start, bytes
  for (var i=0; i<seg.curBitfieldBuf.length; i+=1) {
    if (seg.curBitfieldBuf[i] !== seg.oldBitfieldBuf[i]) {
      start = i
      //      while (seg.curBitfieldBuf[i] !== seg.oldBitfieldBuf[i]) {
      do {
//        console.warn("seg.curBitfieldBuf[%d] = %d", i, seg.curBitfieldBuf[i])
//        console.warn("seg.oldBitfieldBuf[%d] = %d", i, seg.oldBitfieldBuf[i])
        i += 1
      } while (seg.curBitfieldBuf[i] !== seg.oldBitfieldBuf[i])
      //      }
      bytes = seg.curBitfieldBuf.slice(start, i)
//      console.warn("bytes = %j", bytes)
      bytesChanged.push([ start, bytes ])
    }
  }

//  console.warn("bytesChange = %j", bytesChanged)

  if (!seg.fresh) { //fresh means it's never been written to disk
    async.series(
      [ function(scb){
          async.eachSeries(
            bytesChanged
          , function(pair, next){
              var start  = pair[0]
                ...
                , fsmOff = self._calcSegOff(segNum)+self.props.fsmOffsetPrimary()
//              console.warn("\nfs.write( %d //self.fd\n"+
//                           "  o      , seg.curBitfieldBuf\n"+
//                           "        , %d //start\n"+
//                           "        , %d //length\n"+
//                           "        , %d + %d //fsmOff + start\n"+
//                           "        , %s /*next*/)"
//                          , self.fd, start, length, fsmOff, start, typeof next)
              fs.write( self.fd
                      , seg.curBitfieldBuf
                      , start
                      , length
      ...
      , function(scb){
          ...
          , function(pair, next){
              var start  = pair[0]
                , length = pair[1].length
                , fsmOff = self._calcSegOff(segNum)+self.props.fsmOffsetSecondary()
//              console.warn("\nfs.write( %d //self.fd\n"+
//                           "        , seg.curBitfieldBuf\n"+
//                           "        , %d //start\n"+
//                           "        , %d //length\n"+
...
//                          , self.fd, start, length, fsmOff, start, typeof next)
    ...
    , function(err){
        if (err) { cb(err); return }
        for (var i=0; i<bytesChanged.length; i+=1) {
          var pair = bytesChanged[i]
            , start  = pair[0]
            , bytes  = pair[1]
          for (var j=0; j<bytes.length; j+=1)
            seg.oldBitfieldBuf[start+j] = seg.curBitfieldBuf[start+j]
        }
//        assert.ok( bufferEqual(seg.curBitfieldBuf, seg.oldBitfieldBuf)
//                 , "curBitfield != oldBitfield")
        seg.dirty = false
        doneNS()
        cb(null)
      })
    ...
    , function(err){
        if (err) { cb(err); return }
        seg.curBitfieldBuf.copy(seg.oldBitfieldBuf)
        seg.fresh = false
        seg.dirty = false //stupid hack cuz this storage API BLOWS & SUCKS!
        doneNS()
        cb(null)
      })
  }
 ...
 * @param {Function} [cb] cb(err)
 */
BlockFile.prototype._writeSegments = function(cb){
  var self = this

  var segs = u.filter(self.segments, function(seg){ return seg.dirty })
             .reverse()

//  console.warn("BlockFile#writeSegemnts: #dirty segs=%d", segs.length)
 ...
 * @return {Handle}
 */
BlockFile.prototype._reserve = function(numBlks){
  var len, hdl, segNum
