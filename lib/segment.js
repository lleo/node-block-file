...
function Segment(segNum, bitfieldBuf, fsmType, props) {
  this.props = props || Props.defaultProps

  Handle.assertValidSegNum(segNum, this.props)
  assert.ok( Buffer.isBuffer(bitfieldBuf), "argument bitfieldBuf is not a Buffer" )
  assert.strictEqual(bitfieldBuf.length, this.props.fsmSize())
  ...
  for (var i = this.props.minBlkNum(); i <= this.props.maxBlkNum(); i++) {
    this.freeBlockMap[i] = readBit(bitfieldBuf,16+i)
  }
  ...
  this.fresh = true //fresh means it's never been written to disk
  this.dirty = false
  this.curBitfieldBuf = bitfieldBuf //allocated externally
  this.oldBitfieldBuf = new Buffer( bitfieldBuf )
}

Segment.prototype.toString = function(){
  return format( "Segment(segNum=%d, curBitfieldBuf[%d], freeBlockMap[%d], fsm=%s, dirty=%j)"
               , this.segNum, this.curBitfieldBuf.length, this.freeBlockMap.length
...
Segment.prototype.get = function(blkNum){
  ...
  var fbmv = this.freeBlockMap[blkNum]
    ...
    , bufv = readBit(this.curBitfieldBuf, fixedOffset + blkNum)

  assert.equal(fbmv, bufv == 1)
...
Segment.prototype.set = function(blkNum, v){
  ...
  writeBit( this.curBitfieldBuf, fixedOffset + blkNum, v)
 ...
 *
 * @param {Handle} hdl
 * @return {Boolean} indicates whether it was released; false == not released
 */
