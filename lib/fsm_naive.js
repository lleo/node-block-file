...
function NaiveFSM__free(begBlkNum, spanNum) {
  Handle.assertValidBlockNum(begBlkNum)
  ...
  if ( Handle.isValidBlockNum(hiBlkNum) /*in the correct range*/) {
    ...
    if ( hiSpanNum !== undefined ) {
      Handle.assertValidSpanNum(hiSpanNum)
      ...
      assert( idx !== len, format("hiBlkNum(%d) not found in this.spans[%d]"
                                 , hiBlkNum, hiSpanNum))
  ...
  if ( Handle.isValidBlockNum(loBlkNum) /*in the correct range*/) {
    ...
    if ( loSpanNum !== undefined ) {
      Handle.assertValidSpanNum(loSpanNum)
      ...
      //if (idx === len) /*not found*/
      assert( idx !== len, format("loBlkNum(%d) not found in this.spans[%d]"
                                 , loBlkNum, loSpanNum))
...
function NaiveFSM__insert(begBlkNum, spanNum) {
  ...
  for (idx = 0; idx < len; idx += 1) {
    assert.ok(this.spans[spanNum][idx] !== begBlkNum)
